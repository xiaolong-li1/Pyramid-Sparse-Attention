...
def _bwd_kernel_small_32(
    ...
    for row_idx in range(m_block_end):
        ...
        if mask == 1:
            ...
            # DQ accumulation handled in dedicated kernels; only DK/DV here
...
def _bwd_kernel_mask_2(
    ...
    for row_idx in range(m_block_start, m_block_end):
        ...
        if mask == 2:
            ...
            # DQ accumulation handled in dedicated kernels
...
def _bwd_dq_inner_small(
    ...
    p = tl.exp(qk - m[:, None])
    p = tl.where(valid_m[:, None], p, 0.0)

    dp = tl.dot(do, tl.trans(v))
    dp -= Di[:, None]
    dp = tl.where(valid_m[:, None], dp, 0.0)
...
def _bwd_dq_kernel_mask_1(
    ...
    valid_mask = offs_m < N_CTX
    offs_d = tl.arange(0, BLOCK_DMODEL)
    offs_m_tile = tl.arange(0, BLOCK_M_SMALL)
    offs_n_base = tl.arange(0, BLOCK_N_SMALL)

    num_col_blocks = tl.cdiv(N_CTX, BLOCK_N)
    num_m_tiles = tl.cdiv(BLOCK_M, BLOCK_M_SMALL)

    for m_tile_idx in range(num_m_tiles):
        tile_m = start_m + m_tile_idx * BLOCK_M_SMALL + offs_m_tile
        valid_tile = tile_m < N_CTX

        q_tile_ptrs = Q + (tile_m[:, None] * stride_qm + offs_d[None, :] * stride_qd)
        do_tile_ptrs = DO + (tile_m[:, None] * stride_om + offs_d[None, :] * stride_od)
        dq_tile_ptrs = DQ + (tile_m[:, None] * stride_qm + offs_d[None, :] * stride_qd)

        q_tile = tl.load(q_tile_ptrs, mask=valid_tile[:, None], other=0.0).to(tl.float32)
        do_tile = tl.load(do_tile_ptrs, mask=valid_tile[:, None], other=0.0).to(tl.float32)
        dq_tile = tl.load(dq_tile_ptrs, mask=valid_tile[:, None], other=0.0).to(tl.float32)
        m_tile = tl.load(M + tile_m, mask=valid_tile, other=-float("inf")).to(tl.float32)
        Di_tile = tl.load(D + tile_m, mask=valid_tile, other=0.0).to(tl.float32)

        for col_idx in range(num_col_blocks):
            mask_ptr = block_mask_ptr + row_idx * stride_bmm + col_idx * stride_bmn
            mask_val = tl.load(mask_ptr)
            if mask_val == 1:
                start_n = col_idx * BLOCK_N
                for inner_idx in range(BLOCK_N // BLOCK_N_SMALL):
                    offs_n = start_n + inner_idx * BLOCK_N_SMALL + offs_n_base
        ...
        tl.store(dq_tile_ptrs, dq_tile.to(DQ.dtype.element_ty), mask=valid_tile[:, None])


@triton.jit
def _bwd_dq_kernel_mask_2(
    Q, K_2, V_2, sm_scale,
    block_mask_ptr,
    DO, DQ,
    M, D,
    ...
    q = tl.load(q_ptrs, mask=valid_mask[:, None], other=0.0).to(tl.float32)
    do = tl.load(do_ptrs, mask=valid_mask[:, None], other=0.0).to(tl.float32)
    dq_acc = tl.load(dq_ptrs, mask=valid_mask[:, None], other=0.0).to(tl.float32)
    m = tl.load(M + offs_m, mask=valid_mask, other=-float("inf")).to(tl.float32)
    Di = tl.load(D + offs_m, mask=valid_mask, other=0.0).to(tl.float32)

    num_col_blocks = tl.cdiv(N_CTX_2, BLOCK_N_2)
    LOG2: tl.constexpr = 0.6931471805599453
    for col_idx in range(num_col_blocks):
        mask_ptr = block_mask_ptr + row_idx * stride_bmm + col_idx * stride_bmn
        mask_val = tl.load(mask_ptr)
        if mask_val == 2:
            offs_n = col_idx * BLOCK_N_2 + tl.arange(0, BLOCK_N_2)
            k_ptrs = K_2 + (offs_n[:, None] * stride_kn_2 + offs_d[None, :] * stride_kd_2)
            v_ptrs = V_2 + (offs_n[:, None] * stride_vn_2 + offs_d[None, :] * stride_vd_2)
            k = tl.load(k_ptrs, mask=offs_n[:, None] < N_CTX_2, other=0.0)
            v = tl.load(v_ptrs, mask=offs_n[:, None] < N_CTX_2, other=0.0)

            qk = tl.dot(q, tl.trans(k.to(tl.float32))) * sm_scale
            qk += LOG2
            valid_n = offs_n[None, :] < N_CTX_2
    ...
    tl.store(dq_ptrs, dq_acc.to(DQ.dtype.element_ty), mask=valid_mask[:, None])


@triton.jit
def _bwd_dq_kernel_mask_4(
    Q, K_4, V_4, sm_scale,
    block_mask_ptr,
    DO, DQ,
    M, D,
    ...
    num_col_blocks = tl.cdiv(N_CTX_4, BLOCK_N_4)
    LOG4: tl.constexpr = 1.3862943611198906
    for col_idx in range(num_col_blocks):
        mask_ptr = block_mask_ptr + row_idx * stride_bmm + col_idx * stride_bmn
        mask_val = tl.load(mask_ptr)
        if mask_val == 4:
            offs_n = col_idx * BLOCK_N_4 + tl.arange(0, BLOCK_N_4)
            k_ptrs = K_4 + (offs_n[:, None] * stride_kn_4 + offs_d[None, :] * stride_kd_4)
            v_ptrs = V_4 + (offs_n[:, None] * stride_vn_4 + offs_d[None, :] * stride_vd_4)
            k = tl.load(k_ptrs, mask=offs_n[:, None] < N_CTX_4, other=0.0)
            v = tl.load(v_ptrs, mask=offs_n[:, None] < N_CTX_4, other=0.0)

            qk = tl.dot(q, tl.trans(k.to(tl.float32))) * sm_scale
            qk += LOG4
            valid_n = offs_n[None, :] < N_CTX_4
            qk = tl.where(valid_n, qk, -float("inf"))
            p = tl.exp(qk - m[:, None])
...
def _bwd_dq_kernel_mask_8(
    ...
    num_col_blocks = tl.cdiv(N_CTX_8, BLOCK_N_8)
    LOG8: tl.constexpr = 2.079441541679836
    for col_idx in range(num_col_blocks):
        mask_ptr = block_mask_ptr + row_idx * stride_bmm + col_idx * stride_bmn
        mask_val = tl.load(mask_ptr)
        if mask_val == 8:
            offs_n = col_idx * BLOCK_N_8 + tl.arange(0, BLOCK_N_8)
            k_ptrs = K_8 + (offs_n[:, None] * stride_kn_8 + offs_d[None, :] * stride_kd_8)
            v_ptrs = V_8 + (offs_n[:, None] * stride_vn_8 + offs_d[None, :] * stride_vd_8)
            k = tl.load(k_ptrs, mask=offs_n[:, None] < N_CTX_8, other=0.0)
            v = tl.load(v_ptrs, mask=offs_n[:, None] < N_CTX_8, other=0.0)

            qk = tl.dot(q, tl.trans(k.to(tl.float32))) * sm_scale
            qk += LOG8
            valid_n = offs_n[None, :] < N_CTX_8
            qk = tl.where(valid_n, qk, -float("inf"))
            p = tl.exp(qk - m[:, None])
...
def _backward(ctx, do):
    ...
    grid_dq = (triton.cdiv(q.shape[2], ctx.BLOCK_M), ctx.grid[1])
    _bwd_dq_kernel_mask_1[grid_dq](
        q, k, v, ctx.sm_scale,
        block_sparse_mask,
        do_scaled, dq,
        m, delta,
