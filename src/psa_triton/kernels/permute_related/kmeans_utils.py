...
# Optional cuvs import for GPU-accelerated KMeans
try:
    from cuvs.cluster.kmeans import KMeansParams, fit
    HAS_CUVS = True
except ImportError:
    HAS_CUVS = False
    # Fallback will be used if cuvs is not available
...
def kmeans_rapidai(
    ...
    Note: Requires cuvs-cu12 package to be installed.
    If not available, use batch_kmeans_Euclid() instead.
    """
    if not HAS_CUVS:
        raise ImportError(
            "cuvs is not installed. Install with: pip install cuvs-cu12\n"
            "Or use batch_kmeans_Euclid() as an alternative."
        )
