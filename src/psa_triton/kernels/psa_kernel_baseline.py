...
def _fwd_kernel(
    ...
    offs_n_2 = tl.arange(0, BLOCK_N_2)
    offs_n_4 = tl.arange(0, BLOCK_N_4)
    offs_n_8 = tl.arange(0, BLOCK_N_8)
    offs_n_16 = tl.arange(0, BLOCK_N_16)
    ...
    off_k_16 = offs_n_16[None, :] * stride_kn_16 + offs_d[:, None] * stride_kd_16
    ...
    off_v_16 = offs_n_16[:, None] * stride_vn_16 + offs_d[None, :] * stride_vd_16
    ...
    k_block_end = tl.cdiv(N_CTX, BLOCK_N)  # seq_len_k is the sequence length of K

    # loop over k, v and update accumulator
    for col_idx in range(k_block_start, k_block_end):
        mask = tl.load(mask_ptrs + col_idx * stride_bmn)

        # Check if this is the last block
        is_last_block = col_idx == k_block_end - 1

        if mask == 1:
            acc, l_i, m_i = _fwd_kernel_inner_full(
                acc,
                l_i,
                m_i,
                q,
        ...
        # if(mask == 16):
        #     acc, l_i, m_i = _fwd_kernel_inner_16(
        #         acc, l_i, m_i,
        #         q,
        #         col_idx,
        #         k_ptrs_16, v_ptrs_16,
        #         offs_n_16,
        #         stride_kn_16, stride_vn_16,
        #         sm_scale,
        #         N_CTX_16,
        #         is_last_block,
        #         BLOCK_M,
...
def pooling(x, zoom_ratio):
    """
    Perform pooling operation on input tensor x using average pooling.
    x: [B, H, L, D]
    zoom_ratio: Pooling zoom ratio
    """
    B, H, L, D = x.shape

    # Ensure sequence length is divisible by zoom_ratio, otherwise pad
    remainder = L % zoom_ratio
    if remainder != 0:
        pad_len = zoom_ratio - remainder
        # Pad at the end of sequence dimension, using replicate mode
        x = F.pad(x, (0, 0, 0, pad_len), mode='replicate')
        L = x.shape[2]  # Update length
