...
def _backward(ctx, do):
    ...
    pooling_block_n = ctx.POOLING_BLOCK_N
    compute_tile_n = getattr(ctx, 'COMPUTE_TILE_N', 32)
    grid_backward_1 = (triton.cdiv(k.shape[2], pooling_block_n), ctx.grid[1])
    _bwd_kernel_mask_1[grid_backward_1](
        ...
        q.shape[1], N_CTX,
        BLOCK_M=ctx.BLOCK_M,
        BLOCK_N=pooling_block_n,
        BLOCK_DMODEL=ctx.BLOCK_DMODEL,
        BLOCK_N_SMALL=min(32, compute_tile_n),
        num_warps=ctx.num_warps,
        num_stages=1,
    )

    # Kernel for mask=2 (2x downsampled)
    POOLING_BLOCK_N_2 = max(1, pooling_block_n // 2)
    ...
    POOLING_BLOCK_N_4 = max(1, pooling_block_n // 4)
    ...
    POOLING_BLOCK_N_8 = max(1, pooling_block_n // 8)
