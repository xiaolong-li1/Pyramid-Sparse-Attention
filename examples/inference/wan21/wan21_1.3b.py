"""
Wan2.1-1.3B Inference Script with PSA Support

This script generates videos using Wan2.1-1.3B model with optional Pyramid Sparse Attention (PSA).

Usage:
    # Single prompt with PSA
    python wan21_1.3b.py --prompt "A cat baking a cake" --use_psa

    # Batch inference from file
    python wan21_1.3b.py --prompt_file prompts.txt --batch_size 2
...
def setup_pipeline(config: dict, device: str = "cuda"):
    """Setup Wan2.1-1.3B pipeline with configuration."""
    print(f"\n{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(description="Wan2.1-1.3B Video Generation")
    ...
    parser.add_argument("--height", type=int, help="Video height (default: 480)")
    parser.add_argument("--num_frames", type=int, help="Number of frames (default: 81)")
