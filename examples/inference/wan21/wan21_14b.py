"""
Wan2.1-14B Inference Script with PSA Support

This script generates videos using Wan2.1-14B model with optional Pyramid Sparse Attention (PSA).

Usage:
    # Single prompt with PSA
    python wan21_14b.py --prompt "A cat baking a cake" --use_psa

    # Batch inference from file
    python wan21_14b.py --prompt_file prompts.txt --batch_size 2

    # Custom video resolution
    python wan21_14b.py --prompt "..." --width 1280 --height 768 --num_frames 69
"""
...
def setup_pipeline(config: dict, device: str = "cuda"):
    """Setup Wan2.1-14B pipeline with configuration."""
    print(f"\n{'='*70}")
    print(f"Loading Wan2.1-14B Pipeline")
    print(f"{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(description="Wan2.1-14B Video Generation")
    ...
    # Load configuration
    config = get_model_config("wan21_14b")
